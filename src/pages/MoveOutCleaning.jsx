import React from "react";
import {
  FaMagic,
  FaCheck,
  FaTruck,
  FaPhoneAlt,
  FaClock,
  FaShieldAlt,
  FaPercent,
  FaUtensils,
  FaBath,
  FaDoorOpen,
} from "react-icons/fa";

// Premium visual assets matching your structural patterns
import moveOutCleanHero from "../assets/move-out-cleaning.jpg";
import kitchenPrepImg from "../assets/move-out-cleaning.jpg";
import bathroomSanitizeImg from "../assets/move-out-cleaning.jpg";
import livingReadyImg from "../assets/move-out-cleaning.jpg";

const MoveOutCleaning = () => {
  return (
    <div className="bg-slate-50 font-sans text-gray-700 antialiased overflow-hidden pt-20">
      {/* SECTION 1: DENSE MOVE-OUT CLEANING HERO (Google SEO Optimization Anchor) */}
      <section
        className="relative bg-gradient-to-r from-[#0D2B5C] to-blue-900 text-white pt-28 pb-32 px-6 bg-cover bg-center"
        style={{
          backgroundImage: `linear-gradient(to right, rgba(13, 43, 92, 0.96), rgba(30, 58, 138, 0.88)), url(${moveOutCleanHero})`,
        }}
      >
        {/* Magic Icon Floating Decor */}
        <div className="absolute top-12 right-12 text-amber-400 text-4xl opacity-20 animate-pulse">
          <FaMagic />
        </div>

        <div className="max-w-5xl mx-auto text-center space-y-8 relative z-10">
          <div className="inline-flex items-center gap-2 bg-amber-500/20 text-amber-400 text-xs font-bold uppercase tracking-widest px-4 py-2 rounded-xl border border-amber-500/30 backdrop-blur-md">
            <FaMagic className="animate-pulse text-amber-400" /> 100 %
            besiktningsgaranti • Flyttstädning Stockholm
          </div>

          <h1 className="text-4xl md:text-6xl font-black text-white tracking-tight leading-tight max-w-4xl mx-auto">
            Djupgående flyttstädning <br />
            <span className="text-amber-400">
              Säkra din fulla hyresdeposition
            </span>
          </h1>

          <p className="text-base md:text-xl text-slate-200 max-w-3xl mx-auto font-light leading-relaxed">
            Smidig övergång med Stockholms mest pålitliga ramverk för djupgående
            flyttstädning. Våra specialiserade upplägg följer strikt
            Mäklarsamfundets riktlinjer för att säkerställa en problemfri
            besiktning med din hyresvärd eller nya köpare.
          </p>

          <p className="text-xs md:text-sm text-slate-300 max-w-xl mx-auto font-normal border-t border-white/10 pt-6">
            Inkluderar en omfattande 48-timmars besiktningsgaranti, moderna
            avfettningsmedel och direkt 30 % företagsrabatt.
          </p>
        </div>
      </section>



      {/* SECTION 2: FIXED SQUARE METER PRICING BANNER MATRIX FOR FLYTTSTÄDNING */}
      <section className="bg-amber-500 py-10 px-6 text-blue-950 shadow-inner relative z-20">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center justify-between gap-8">
          {/* Text and Icon Block */}
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 text-center sm:text-left">
            <div className="p-4 bg-[#0D2B5C] rounded-2xl text-2xl text-amber-400 flex-shrink-0 shadow-md">
              <FaPercent className="animate-pulse" />
            </div>
            <div className="space-y-2">
              <h4 className="font-black text-xl md:text-2xl tracking-tight uppercase">
          Fasta priser för flyttstädning med 30 % företagsrabatt
              </h4>
              <p className="text-sm font-semibold text-blue-950/90 max-w-4xl leading-relaxed">
                Stockholm Clean HB tillämpar mycket strukturerade och transparenta prismodeller 
          som beräknas direkt utifrån din fastighets totala kvadratmeter. Vi hanterar 
          all prissättning och rabatter automatiskt via våra egna system – vilket ger noll 
          utrymme för dolda avgifter eller överraskande fakturor.
              </p>
            </div>
          </div>

          {/* Explicit Pricing Grid Badges */}
          <div className="flex-shrink-0 w-full lg:w-auto text-center grid grid-cols-1 sm:grid-cols-2 lg:flex gap-4">
            <div className="bg-[#0D2B5C] text-white p-4 px-6 rounded-xl border border-blue-950 shadow-sm text-center">
              <span className="block text-[10px] uppercase tracking-widest text-amber-400 font-extrabold">
          Standard baspris
              </span>
              <span className="text-lg font-black">Från 80 kr / kvm</span>
            </div>
            <div className="bg-white text-blue-950 p-4 px-6 rounded-xl border border-slate-100 shadow-md text-center">
              <span className="block text-[10px] uppercase tracking-widest text-green-600 font-extrabold">
          Ditt pris med 30 % rabatt
              </span>
              <span className="text-lg font-black text-green-600">
          Från 56 kr / kvm
              </span>
            </div>
          </div>
        </div>
      </section>



      {/* SECTION 3: PART 1 - TITLE & DEEP KITCHEN PREP CARD WITH EXPLICIT PRICING */}
      <section className="py-24 px-6 max-w-7xl mx-auto relative z-10">
        <div className="text-center space-y-4 mb-20">
          <span className="text-xs font-bold uppercase text-amber-500 tracking-widest bg-amber-500/10 px-3 py-1.5 rounded-full border border-amber-500/20 inline-flex items-center gap-1.5">
            <FaMagic className="text-amber-500 animate-pulse" /> Juridiska checklistor
          </span>
          <h2 className="text-3xl md:text-5xl font-black text-blue-950 tracking-tight leading-tight max-w-3xl mx-auto">
                  Specificerade ramverk för flyttstädning med fasta kvadratmeterpriser

          </h2>
          <p className="text-slate-500 max-w-xl mx-auto text-sm md:text-base font-light">
            Granska de rigorösa strukturella uppdelningar som tillämpas av våra lokalvårdare. 
      Varje zon följer regionala standarder för besiktning på ett fläckfritt sätt.
          </p>
        </div>
        {/* Start of 3-Column Move-Out Cleaning Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          {/* CATEGORY 1: DEEP KITCHEN PREP */}
          <div className="bg-white rounded-3xl overflow-hidden border border-slate-200/80 shadow-md hover:shadow-2xl transition-all duration-500 flex flex-col group">
            <div className="h-64 bg-slate-200 overflow-hidden relative">
              <img
                src={kitchenPrepImg}
                alt="Intensive Deep Kitchen Preparation detailing Stove deep clean, Inside drawers, Appliance exterior, and Backsplash scrubbing"
                className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"
              />
              <span className="absolute top-4 right-4 bg-[#0D2B5C] text-white text-xs font-bold px-3 py-1.5 rounded-xl shadow-sm flex items-center gap-1">
                <FaUtensils className="text-amber-400" /> Djuprengöring av kök
              </span>
            </div>
            <div className="p-8 flex-1 flex flex-col justify-between space-y-6">
              <div className="space-y-4">
                <div className="flex justify-between items-baseline border-b border-slate-100 pb-3">
                  <h3 className="text-2xl font-black text-blue-950 tracking-tight">
                    Köksbesiktning
                  </h3>
                  <span className="text-xs font-extrabold text-blue-950 bg-blue-50 px-2.5 py-1 rounded-md">
Djuprengöring                  </span>
                </div>
                <p className="text-slate-500 text-sm leading-relaxed font-light">
                 Anpassad högintensiv modul för fettborttagning med stort fokus på 
            vitvarukanter och kritiska ytor för matberedning.
                </p>
                {/* Specific Target Checklist Nodes */}
                <div className="space-y-2.5 pt-2">
                  <div className="flex items-center gap-3 text-sm text-slate-700 font-semibold">
                    <span className="w-5 h-5 rounded-md bg-amber-500/10 text-amber-600 flex items-center justify-center text-xs">
                      ✓
                    </span>
              <span>Djupgående invändig rengöring av spis & brännare</span>
                  </div>
                  <div className="flex items-center gap-3 text-sm text-slate-700 font-semibold">
                    <span className="w-5 h-5 rounded-md bg-amber-500/10 text-amber-600 flex items-center justify-center text-xs">
                      ✓
                    </span>
              <span>Rengöring av lådor invändigt & inbyggda skåp</span>
                  </div>
                  <div className="flex items-center gap-3 text-sm text-slate-700 font-semibold">
                    <span className="w-5 h-5 rounded-md bg-amber-500/10 text-amber-600 flex items-center justify-center text-xs">
                      ✓
                    </span>
              <span>Komplett utvändig rengöring & polering av vitvaror</span>
                  </div>
                  <div className="flex items-center gap-3 text-sm text-slate-700 font-semibold">
                    <span className="w-5 h-5 rounded-md bg-amber-500/10 text-amber-600 flex items-center justify-center text-xs">
                      ✓
                    </span>
              <span>Intensiv skrubbning av kakel och stänkskydd</span>
                  </div>
                </div>
              </div>
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 text-center">
                <span className="block text-[10px] uppercase font-bold text-slate-400">
            Baspris för denna kategori
                </span>
                <span className="text-xl font-black text-blue-950">
            Från 80 kr / kvm
                </span>
              </div>
            </div>
          </div>

          {/* CATEGORY 2: DEEP BATHROOM SANITIZE WITH EXPLICIT PRICING */}
          <div className="bg-white rounded-3xl overflow-hidden border border-slate-200/80 shadow-md hover:shadow-2xl transition-all duration-500 flex flex-col group">
            <div className="h-64 bg-slate-200 overflow-hidden relative">
              <img
                src={bathroomSanitizeImg}
                alt="Deep Bathroom Sanitization and sterilization tracking Tub descaling, Tile bleaching, Mirror polishing, and Exhaust fan dusting"
                className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"
              />
              <span className="absolute top-4 right-4 bg-[#0D2B5C] text-white text-xs font-bold px-3 py-1.5 rounded-xl shadow-sm flex items-center gap-1">
                <FaBath className="text-amber-400" /> Badrumsmodul
              </span>
            </div>
            <div className="p-8 flex-1 flex flex-col justify-between space-y-6">
              <div className="space-y-4">
                <div className="flex justify-between items-baseline border-b border-slate-100 pb-3">
                  <h3 className="text-2xl font-black text-blue-950 tracking-tight">
                    Badrumssanering
                  </h3>
                  <span className="text-xs font-extrabold text-blue-950 bg-blue-50 px-2.5 py-1 rounded-md">
                    Deep Clean
                  </span>
                </div>
                <p className="text-slate-500 text-sm leading-relaxed font-light">
                  Intensivt avlägsnande av envisa kalkavlagringar, urinsten och djupgående 
        bakterielager i alla badrumsutrymmen.
                </p>
                {/* Specific Target Checklist Nodes */}
                <div className="space-y-2.5 pt-2">
                  <div className="flex items-center gap-3 text-sm text-slate-700 font-semibold">
                    <span className="w-5 h-5 rounded-md bg-amber-500/10 text-amber-600 flex items-center justify-center text-xs">
                      ✓
                    </span>
          <span>Avkalkning av badkar, duschkabin & blandare</span>
                  </div>
                  <div className="flex items-center gap-3 text-sm text-slate-700 font-semibold">
                    <span className="w-5 h-5 rounded-md bg-amber-500/10 text-amber-600 flex items-center justify-center text-xs">
                      ✓
                    </span>
          <span>Rengöring av kakel & intensiv skrubbning av fogar</span>
                  </div>
                  <div className="flex items-center gap-3 text-sm text-slate-700 font-semibold">
                    <span className="w-5 h-5 rounded-md bg-amber-500/10 text-amber-600 flex items-center justify-center text-xs">
                      ✓
                    </span>
          <span>Polering & putsning av speglar samt glasytor</span>
                  </div>
                  <div className="flex items-center gap-3 text-sm text-slate-700 font-semibold">
                    <span className="w-5 h-5 rounded-md bg-amber-500/10 text-amber-600 flex items-center justify-center text-xs">
                      ✓
                    </span>
          <span>Rengöring av frånluftsfläkt & avtorkning av ventilationsdon</span>
                  </div>
                </div>
              </div>
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 text-center">
                <span className="block text-[10px] uppercase font-bold text-slate-400">
        Baspris för denna kategori
                </span>
                <span className="text-xl font-black text-blue-950">
        Från 80 kr / kvm
                </span>
              </div>
            </div>
          </div>


          {/* CATEGORY 3: LIVING AREAS READY WITH EXPLICIT PRICING */}
          <div className="bg-white rounded-3xl overflow-hidden border border-slate-200/80 shadow-md hover:shadow-2xl transition-all duration-500 flex flex-col group">
            <div className="h-64 bg-slate-200 overflow-hidden relative">
              <img
                src={livingReadyImg}
                alt="Move-Out Living Areas Ready detailing Baseboard washing, Light switches wiping, Closet inside cleaning, and Door frame wiping"
                className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"
              />
              <span className="absolute top-4 right-4 bg-[#0D2B5C] text-white text-xs font-bold px-3 py-1.5 rounded-xl shadow-sm flex items-center gap-1">
                <FaDoorOpen className="text-amber-400" /> Living Areas Ready
              </span>
            </div>
            <div className="p-8 flex-1 flex flex-col justify-between space-y-6">
              <div className="space-y-4">
                <div className="flex justify-between items-baseline border-b border-slate-100 pb-3">
                  <h3 className="text-2xl font-black text-blue-950 tracking-tight">
                    Bostadsytor
                  </h3>
                  <span className="text-xs font-extrabold text-blue-950 bg-blue-50 px-2.5 py-1 rounded-md">
                    Djuprengöring
                  </span>
                </div>
                <p className="text-slate-500 text-sm leading-relaxed font-light">
                  Noggrann invändig rengöring utformad för att torka bort fläckar, 
                  fingeravtryck och dammlager från alla högfrekventa kontaktytor.
                </p>
                {/* Specific Target Checklist Nodes */}
                <div className="space-y-2.5 pt-2">
                  <div className="flex items-center gap-3 text-sm text-slate-700 font-semibold">
                    <span className="w-5 h-5 rounded-md bg-amber-500/10 text-amber-600 flex items-center justify-center text-xs">
                      ✓
                    </span>
                    <span>Aftorkning & rengöring av golvlister & lister</span>
                  </div>
                  <div className="flex items-center gap-3 text-sm text-slate-700 font-semibold">
                    <span className="w-5 h-5 rounded-md bg-amber-500/10 text-amber-600 flex items-center justify-center text-xs">
                      ✓
                    </span>
                    <span>Rengöring av strömbrytare & eluttag</span>
                  </div>
                  <div className="flex items-center gap-3 text-sm text-slate-700 font-semibold">
                    <span className="w-5 h-5 rounded-md bg-amber-500/10 text-amber-600 flex items-center justify-center text-xs">
                      ✓
                    </span>
                    <span>Djupgående invändig dammtorkning av garderober & skåp</span>
                  </div>
                  <div className="flex items-center gap-3 text-sm text-slate-700 font-semibold">
                    <span className="w-5 h-5 rounded-md bg-amber-500/10 text-amber-600 flex items-center justify-center text-xs">
                      ✓
                    </span>
                    <span>Avtorkning av dörrkarmar, dörrblad & fläckborttagning</span>
                  </div>
                </div>
              </div>
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 text-center">
                <span className="block text-[10px] uppercase font-bold text-slate-400">
                  Baspris för denna kategori
                </span>
                <span className="text-xl font-black text-blue-950">
                  Från 80 kr / kvm
                </span>
              </div>
            </div>
          </div>
        </div>{" "}
        {/* Closing the grid layout safely */}
      </section>



      {/* SECTION 4: STOCKHOLM RELOCATION TRANSIT GEO-TARGETING BOOSTER */}
      <section className="bg-slate-100 py-24 px-6 border-y border-slate-200 relative z-10">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          {/* Left Text Block */}
          <div className="lg:col-span-5 space-y-6">
            <div className="space-y-2">
              <span className="text-xs font-bold uppercase text-amber-500 tracking-widest bg-amber-500/10 px-3 py-1.5 rounded-full border border-amber-500/20 inline-flex items-center gap-1.5">
                <FaMagic className="text-amber-500 animate-pulse" /> Logistik för flyttstädning
              </span>
              <h2 className="text-3xl md:text-5xl font-black text-blue-950 tracking-tight leading-tight">
          Djupgående flyttstädning över hela Stockholms län
              </h2>
            </div>
            <p className="text-slate-500 text-sm md:text-base leading-relaxed font-light">
              Våra mobila och professionella städteam är systematiskt utplacerade nära 
        större pendlingslinjer i Stockholms län. Denna logistiska närvaro garanterar 
        snabb service och noll extra framkörningsavgifter för våra kunder.
            </p>
            <p className="text-slate-500 text-xs md:text-sm leading-relaxed font-normal">
              Oavsett om du flyttar ut från en studiobostad i innerstaden eller säljer 
        en flervåningsvilla på landsbygden, säkerställer Stockholm Clean HB att din 
        fastighet matchar alla lokala fastighetskrav helt och hållet.
            </p>
          </div>

          {/* Right Area Grid Block */}
          <div className="lg:col-span-7 grid grid-cols-2 sm:grid-cols-3 gap-4">
            <div className="p-5 bg-white rounded-xl border border-slate-200 text-center shadow-sm hover:border-[#0D2B5C] transition-all duration-300 group">
              <span className="block font-extrabold text-blue-950 text-sm group-hover:text-[#0D2B5C]">
                Södermalm
              </span>
              <span className="text-[10px] text-slate-400 font-medium">
          Innergårdsbesiktningar
              </span>
            </div>
            <div className="p-5 bg-white rounded-xl border border-slate-200 text-center shadow-sm hover:border-[#0D2B5C] transition-all duration-300 group">
              <span className="block font-extrabold text-blue-950 text-sm group-hover:text-[#0D2B5C]">
          Vasastan
              </span>
              <span className="text-[10px] text-slate-400 font-medium">
          Stadshandover
              </span>
            </div>
            <div className="p-5 bg-white rounded-xl border border-slate-200 text-center shadow-sm hover:border-[#0D2B5C] transition-all duration-300 group">
              <span className="block font-extrabold text-blue-950 text-sm group-hover:text-[#0D2B5C]">
                Östermalm
              </span>
              <span className="Premium Real Estate Cleans">
          Premium överlämningar
              </span>
            </div>
            <div className="p-5 bg-white rounded-xl border border-slate-200 text-center shadow-sm hover:border-[#0D2B5C] transition-all duration-300 group">
              <span className="block font-extrabold text-blue-950 text-sm group-hover:text-[#0D2B5C]">
                Solna & Sundbyberg
              </span>
              <span className="text-[10px] text-slate-400 font-medium">
          Norra flyttsektorn
              </span>
            </div>
            <div className="p-5 bg-white rounded-xl border border-slate-200 text-center shadow-sm hover:border-[#0D2B5C] transition-all duration-300 group">
              <span className="block font-extrabold text-blue-950 text-sm group-hover:text-[#0D2B5C]">
                Kungsholmen
              </span>
              <span className="text-[10px] text-slate-400 font-medium">
          Västra lägenhetsblocken
              </span>
            </div>
            <div className="p-5 bg-white rounded-xl border border-slate-200 text-center shadow-sm hover:border-[#0D2B5C] transition-all duration-300 group">
              <span className="block font-extrabold text-blue-950 text-sm group-hover:text-[#0D2B5C]">
                Täby & Danderyd
              </span>
              <span className="text-[10px] text-slate-400 font-medium">
          Villaförstäder
              </span>
            </div>
            <div className="p-5 bg-white rounded-xl border border-slate-200 text-center shadow-sm hover:border-[#0D2B5C] transition-all duration-300 group">
              <span className="block font-extrabold text-blue-950 text-sm group-hover:text-[#0D2B5C]">
                Nacka Strand
              </span>
              <span className="text-[10px] text-slate-400 font-medium">
          Östkustens distrikt
              </span>
            </div>
            <div className="p-5 bg-white rounded-xl border border-slate-200 text-center shadow-sm hover:border-[#0D2B5C] transition-all duration-300 group">
              <span className="block font-extrabold text-blue-950 text-sm group-hover:text-[#0D2B5C]">
                Bromma & Alvik
              </span>
              <span className="text-[10px] text-slate-400 font-medium">
          Västra bostadssektorn
              </span>
            </div>
            <div className="p-5 bg-white rounded-xl border border-slate-200 text-center shadow-sm hover:border-[#0D2B5C] transition-all duration-300 group">
              <span className="block font-extrabold text-blue-950 text-sm group-hover:text-[#0D2B5C]">
                Huddinge
              </span>
              <span className="text-[10px] text-slate-400 font-medium">
          Södra storstadsområdet
              </span>
            </div>
          </div>
        </div>
      </section>



      {/* SECTION 5: MOVE-OUT QUOTE SUBMISSION FORM & COMPONENT CLOSURE */}
      <section className="py-24 px-6 max-w-5xl mx-auto relative z-10">
        <div className="bg-white p-8 md:p-12 rounded-3xl shadow-2xl border border-slate-200/60 relative overflow-hidden">
          {/* Aesthetic background mesh nodes */}
          <div className="absolute top-0 right-0 w-64 h-64 bg-amber-500 rounded-full filter blur-[100px] opacity-10 pointer-events-none"></div>

          <div className="space-y-3 mb-10 text-center sm:text-left">
            <span className="text-xs font-bold uppercase text-amber-500 tracking-widest bg-amber-500/10 px-3 py-1 rounded-md inline-flex items-center gap-1">
              <FaMagic className="text-amber-500 lifestyle-pulse text-[10px]" />{" "}
        Verifierad portal för flyttstädning (B2C)
            </span>
            <h3 className="text-2xl md:text-4xl font-black text-blue-950 tracking-tight">
        Begär en kostnadsfri prisförfrågan för flyttstädning direkt
            </h3>
            <p className="text-slate-500 text-sm md:text-base font-light leading-relaxed max-w-3xl">
              Redo att säkra din fulla hyresdeposition inför överlämningen? Skicka in specifikationerna 
        för ditt hem nedan. Vår planeringsavdelning i Stockholm granskar din förfrågan omedelbart 
        för att ta fram tydliga faktureringsunderlag med godkänd F-skatt.
            </p>
          </div>

          <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                <label className="block text-xs font-bold uppercase text-slate-500 mb-2 tracking-wide">
            Ditt fullständiga namn
                </label>
                <input
                  type="text"
                  placeholder="e.g., Erik Johansson"
                  className="w-full px-4 py-3.5 bg-slate-50 rounded-xl border border-slate-200 text-sm font-medium focus:outline-none focus:bg-white focus:border-blue-950 transition-colors"
                />
              </div>
              <div>
                <label className="block text-xs font-bold uppercase text-slate-500 mb-2 tracking-wide">
            Primär e-postadress
                </label>
                <input
                  type="email"
                  placeholder="e.g., erik@gmail.se"
                  className="w-full px-4 py-3.5 bg-slate-50 rounded-xl border border-slate-200 text-sm font-medium focus:outline-none focus:bg-white focus:border-blue-950 transition-colors"
                />
              </div>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              <div className="sm:col-span-2">
                <label className="block text-xs font-bold uppercase text-slate-500 mb-2 tracking-wide">
            Mobiltelefonnummer
                </label>
                <input
                  type="tel"
                  placeholder="e.g., 073 123 45 67"
                  className="w-full px-4 py-3.5 bg-slate-50 rounded-xl border border-slate-200 text-sm font-medium focus:outline-none focus:bg-white focus:border-blue-950 transition-colors"
                />
              </div>
              <div>
                <label className="block text-xs font-bold uppercase text-slate-500 mb-2 tracking-wide">
            Total bostadsstorlek (kvm)
                </label>
                <input
                  type="number"
                  placeholder="e.g., 90"
                  className="w-full px-4 py-3.5 bg-slate-50 rounded-xl border border-slate-200 text-sm font-bold text-blue-950 focus:outline-none focus:bg-white focus:border-blue-950 transition-colors"
                />
              </div>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                <label className="block text-xs font-bold uppercase text-slate-500 mb-2 tracking-wide">
            Typ av bostadsprofil
                </label>
                <select className="w-full px-4 py-3.5 bg-slate-50 rounded-xl border border-slate-200 text-sm font-bold text-blue-950 focus:outline-none focus:bg-white focus:border-blue-950 transition-colors">
                  <option>Hyresrätt (Lägenhet)</option>
            <option>Bostadsrätt (Lägenhet)</option>
            <option>Fristående villa / Kedjehus</option>
            <option>Kommersiell kontorsflytt</option>
                </select>
              </div>
              <div>
                <label className="block text-xs font-bold uppercase text-slate-500 mb-2 tracking-wide">
            Önskat tidsfönster för tjänsten
                </label>
                <select className="w-full px-4 py-3.5 bg-slate-50 rounded-xl border border-slate-200 text-sm font-bold text-blue-950 focus:outline-none focus:bg-white focus:border-blue-950 transition-colors">
                    <option>Standard servicetillfälle (Inom 3–5 dagar)</option>
            <option>Brådskande överlämning (Inom nästa 48 timmar)</option>
            <option>Flexibel bokningsslott vid månadsskifte</option>
                </select>
              </div>
            </div>

            <div className="pt-4 flex flex-col sm:flex-row items-center justify-between gap-6 border-t border-slate-100 mt-8">
              <div className="text-center sm:text-left space-y-1">
                <span className="block text-xs text-slate-400 font-medium">
            Behöver du omedelbar rådgivning inför flytten?
                </span>
                <a
                  href="tel:+46721516306"
                  className="inline-flex items-center gap-2 text-sm font-black text-blue-950 hover:text-amber-500 transition-colors"
                >
                  <FaPhoneAlt className="text-xs text-amber-500" />  Ring vår supportlinje för flyttstädning
                </a>
              </div>
              <button
                type="submit"
                className="w-full sm:w-auto px-10 py-4 bg-[#0D2B5C] text-white font-black text-sm uppercase tracking-widest rounded-xl hover:bg-amber-500 hover:text-blue-950 hover:shadow-xl transition-all duration-300"
              >
          Boka prisförfrågan
              </button>
            </div>
          </form>
        </div>
      </section>
    </div>
  );
};

export default MoveOutCleaning;
